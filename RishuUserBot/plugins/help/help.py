...
from RishuUserBot import app, CMD_HELP
from RishuUserBot.helper.PyroHelpers import ReplyCheck
from RishuUserBot.helper.utility import split_list
    ...
    if len(cmd) > 1:
        ...
        try:
            ...
            xx = await client.send_message(
                message.chat.id,
                f"```{str(ac)}```\n• @lllll_BADNAM_BABY_lllll × @lllll_BADNAM_BABY_lllll •",
...
async def module_helper(client: Client, message: Message):
    ...
    if len(cmd) > 1:
        ...
        ac = PrettyTable()
        ac.header = False
        ac.title = "RishuUserBot-UserBot Plugins"
        ac.align = "l"
        for x in split_list(sorted(CMD_HELP.keys()), 2):
            ac.add_row([x[0], x[1] if len(x) >= 2 else None])
        await edit_or_reply(
    ...
    if help_arg:
        if help_arg in CMD_HELP:
            ...
            for x in commands:
                this_command += f"  •  **Command:** `.{str(x)}`\n  •  **Function:** `{str(commands[x])}`\n\n"
            this_command += "© @lllll_BADNAM_BABY_lllll"
            await edit_or_reply(
                message, this_command, parse_mode=enums.ParseMode.MARKDOWN
            )
        else:
